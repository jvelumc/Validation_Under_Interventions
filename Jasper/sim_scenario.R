...
censoring <- FALSE; 
cindex_ylim_low <- .48; 
cindex_ylim_high <- .625; 
auc_ylim_low <- 0.46; 
auc_ylim_high <- 0.70; 
calib_lim_low <- 0.40; 
calib_lim_high <- 0.95; 
calib_risk_lim_high <- 0.85; 
model <- " additive hazards model"

#------------------------------
#------------------------------
#simulates longitudinal data on treatment A and covariates L at 5 time points,
#and then generates event times according to an additive hazards model. U is an
#individual frailty. People who do not have the event are censored at time 5.
#
